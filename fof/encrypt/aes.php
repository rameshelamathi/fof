...
class F0FEncryptAes
{
	 ...
	 * @param   F0FUtilsPhpfunc  $phpfunc   For testing
	 */
	public function __construct($key, $strength = 128, $mode = 'cbc', F0FUtilsPhpfunc $phpfunc = null)
	{
		$this->adapter = new F0FEncryptAesOpenssl();

		if (!$this->adapter->isSupported($phpfunc))
		{
			$this->adapter = new F0FEncryptAesMcrypt();
		}
	 ...
	 *
	 * @param   F0FUtilsPhpfunc  $phpfunc
	 *
	 * @return boolean
	 */
	public static function isSupported(F0FUtilsPhpfunc $phpfunc = null)
	{
		if (!is_object($phpfunc) || !($phpfunc instanceof $phpfunc))
		{
			$phpfunc = new F0FUtilsPhpfunc();
		}

		$adapter = new F0FEncryptAesMcrypt();

		if (!$adapter->isSupported($phpfunc))
		{
			$adapter = new F0FEncryptAesOpenssl();
		}
