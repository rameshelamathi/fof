...
JS;
		...
		if ($show_header)
		{
			foreach ($headerFields as $headerField)
			{
				...
				if ($sortable)
				{
					$sortFields[$headerField->name] = Text::_($headerField->label);
				}
				...
				if (!empty($filter))
				{
					...
					$filter_html .= Text::_($headerField->label);
					$filter_html .= "</label>\n";
					$filter_html .= "\t$filter\n";
					$filter_html .= "</div>\n";
		...
		$actionUrl        = $this->container->platform->isBackend() ? 'index.php' : Uri::root() . 'index.php';
		...
		if ($this->container->platform->isFrontend() && ($this->container->input->getCmd('Itemid', 0) != 0))
		{
			...
			$actionUrl = Route::_($uri->toString());
		}
		...
		if ($show_header)
		{
			...
			if ($show_pagination)
			{
				...
				$html .= "\t\t" . '<label for="limit" class="element-invisible">' . Text::_('JFIELD_PLG_SEARCH_SEARCHLIMIT_DESC') . '</label>' . "\n";
			...
			if (!empty($sortFields))
			{
				...
				$html     .= "\t" . '<div class="btn-group pull-right hidden-phone">' . "\n";
				$html     .= "\t\t" . '<label for="directionTable" class="element-invisible">' . Text::_('JFIELD_ORDERING_DESC') . '</label>' . "\n";
				$html     .= "\t\t" . '<select name="directionTable" id="directionTable" class="input-medium" onchange="Joomla.orderTable()">' . "\n";
				$html     .= "\t\t\t" . '<option value="">' . Text::_('JFIELD_ORDERING_DESC') . '</option>' . "\n";
				$html     .= "\t\t\t" . '<option value="asc" ' . $asc_sel . '>' . Text::_('JGLOBAL_ORDER_ASCENDING') . '</option>' . "\n";
				$html     .= "\t\t\t" . '<option value="desc" ' . $desc_sel . '>' . Text::_('JGLOBAL_ORDER_DESCENDING') . '</option>' . "\n";
				$html     .= "\t\t" . '</select>' . "\n";
				$html     .= "\t" . '</div>' . "\n\n";

				// Display the sort fields
				$html .= "\t" . '<div class="btn-group pull-right">' . "\n";
				$html .= "\t\t" . '<label for="sortTable" class="element-invisible">' . Text::_('JGLOBAL_SORT_BY') . '</label>' . "\n";
				$html .= "\t\t" . '<select name="sortTable" id="sortTable" class="input-medium" onchange="Joomla.orderTable()">' . "\n";
				$html .= "\t\t\t" . '<option value="">' . Text::_('JGLOBAL_SORT_BY') . '</option>' . "\n";
				$html .= "\t\t\t" . HTMLHelper::_('select.options', $sortFields, 'value', 'text', $form->getView()->getLists()->order) . "\n";
				$html .= "\t\t" . '</select>' . "\n";
				$html .= "\t" . '</div>' . "\n";
			}
		...
		elseif ($norows_placeholder)
		{
			...
			$html .= Text::_($norows_placeholder);
			$html .= "</td></tr>\n";
		}
	...
	public function renderFormEdit(Form &$form, DataModel $model)
	{
		...
		$platform  = $this->container->platform;
		$actionUrl = $platform->isBackend() ? 'index.php' : Uri::root() . 'index.php';
	...
	public function renderFieldset(stdClass &$fieldset, Form &$form, DataModel $model, $formType, $showHeader = true, &$innerHtml = null)
	{
		...
		{
			$html .= "\t\t" . '<h3>' . Text::_($fieldset->label) . '</h3>' . "\n";
		}
		...
		if (!empty($sourceTemplate))
		{
			...
			$html .= $viewObject->loadAnyTemplate($sourceTemplate, [
				'model'     => $model,
				'form'      => $form,
				'fieldset'  => $fieldset,
				'formType'  => $formType,
		...
		if (!empty($fields))
		{
			foreach ($fields as $field)
			{
				...
				{
					$prependText = Text::_($prependText);
				}

				if (!empty($appendText))
				{
					$appendText = Text::_($appendText);
				}
...
JS;
	...
	function renderForm(Form &$form, DataModel $model, $formType = null, $raw = false)
	{
		...
		$useChosen = in_array($useChosen, ['false', 'no', 'off', '0']) ? '' : $useChosen;

		if ($useChosen)
		{
			HTMLHelper::_('formbehavior.chosen', $useChosen);
		}
	...
	{
		$message = $form->getView()->escape(Text::_('JGLOBAL_VALIDATION_FORM_FAILED'));
